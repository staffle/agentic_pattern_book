...
# Import book configuration
from book_config import DEFAULT_CONFIG, get_config
...
def extract_toc_headings_predefined(config=None):
    """Extract predefined TOC headings for the current book from configuration."""
    if config is None:
        config = DEFAULT_CONFIG
    headings = config.get("predefined_headings", [])
    logging.info(f"Extracted {len(headings)} predefined headings from configuration.")
    return headings


def get_book_headings(config=None):
    """Get headings for table of contents.
    
    For now, uses predefined headings from configuration.
    In the future, this could be extended to extract headings from PDF content.
    """
    logging.info("Using predefined TOC headings from provided content.")
    return extract_toc_headings_predefined(config)
...
def add_toc_clickable_links(toc_pdf:Path, final_pdf:Path, heading_page_pairs:list[tuple[str,int]], workdir:Path, toc_start:int=1):
...
def main():
    ...
    # First parse for book config selection
    parser = argparse.ArgumentParser(add_help=False)
    parser.add_argument('--book-config', default=None, help='Book configuration to use (e.g., "agentic")')
    args, remaining = parser.parse_known_args()
    
    # Get the appropriate configuration
    config = get_config(args.book_config)
    
    # Now parse all arguments with the selected configuration defaults
    ap=argparse.ArgumentParser(description=f'Compile {config["title"]} book')
    ap.add_argument('--book-config', default=None, help='Book configuration to use')
    ap.add_argument('--index-pdf', default=config["default_index"])
    ap.add_argument('--cover', default=config["default_cover"])
    ap.add_argument('--out', default=config["default_output"])
    ap.add_argument('--workdir', default=config["default_workdir"])
    ...
    headings=get_book_headings(config)
    ...
    if headings:
        ...
        basic_headings = config.get("fallback_headings", ["Introduction", "Core Concepts", "Implementation Guide"])
        ...
        if headings:
            ...
            try:
                if HAVE_FITZ and headings and 'heading_page_pairs' in locals():
                    logging.info(f"Adding clickable TOC links for {len(heading_page_pairs)} headings")
                    add_toc_clickable_links(toc_pdf, out_pdf, heading_page_pairs, workdir, toc_start=1)
